...
import robocup.Message;
import robocup.SenseMemory;
...
public class Parser {
	...
	/**
	 * This parses the (init) message, the first message sent by the server,
	 * directly after a new Player is initialized.
	 *
	 ...
	 * @post Vital information about the Player will be saved, such as the side
	 * of the field the player starts on, the Player's uniform number and the
	 * play mode, which is "before_kickoff."
	 */
	 ...
	 * @pre A Memory must be created and passed in, along with the message from
	 * the server
	 * @post The message will be parsed and stored either as SenseInfos from the
	 * (sense_body) message, or ObjInfos from the (see) message, or the playMode
	 * from the referee (hear) message
	 */
	 ...
	 *
	 * @param seeArray The ArrayList that stores all the ObjInfos from the
	 * message. This will be saved in the Memory
	 * @param splitPacket The message String split up by parentheses
	 * @post The (see) message will be parsed and the ObjInfos will be stored in
	 * an ArrayList
	 */
	 ...
	 *
	 * @param splitName this is the split up ObjName with flagName, flagType,
	 * side, x_pos, y_pos, and yard
	 * @param splitInfo The rest of the information to store in the ObjLine
	 ...
	 *
	 * @param splitName this is the split up ObjName, team name, uniform number
	 * (if visible), and goalie boolean (if visible).
